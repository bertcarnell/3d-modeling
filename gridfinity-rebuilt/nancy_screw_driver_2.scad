    ...
    union() {
        ...
        difference(){
            color("red") 
                ...
                cube([block_w, gridy*my_grid, h6]);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + h1 + spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
                cylinder(h = block_w + 2, r = h1, center = false);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + 2*h1 + h2 + 2*spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
                cylinder(h = block_w + 2, r = h2, center = false);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + 2*h1 + 2*h2 + h3 + 3*spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
                cylinder(h = block_w + 2, r = h3, center = false);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + 2*h1 + 2*h2 + 2*h3 + h4 + 4*spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
                cylinder(h = block_w + 2, r = h4, center = false);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + 2*h1 + 2*h2 + 2*h3 + 2*h4 + h5 + 5*spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
                cylinder(h = block_w + 2, r = h5, center = false);
            translate([-(gridx-1)/2*my_grid - 1, -gridy/2*my_grid + 2*h1 + 2*h2 + 2*h3 + 2*h4 + 2*h5 + h6 + 6*spacer, h6 + 7]) 
                rotate([0, 90, 0]) 
